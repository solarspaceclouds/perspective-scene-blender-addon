...
# to change camera clip end value
...
class ADDONNAME_PT_TemplatePanel(bpy.types.Panel):
    ...
    bl_idname = "Generate Scene"
    bl_space_type = "VIEW_3D"
    bl_region_type = 'UI'
    bl_category = "Perspective Scene Template"
...
#    particle_scale : bpy.props.StringProperty(
#        name="sometext",
#        description=":",
#        default="",
#        maxlen=1024,
...
#    env_template_enum : bpy.props.EnumProperty(
#        name = "environment",
#        description = "Select an option",
#        items = [
#            ('OP1', 'env1', 'Make this an open field environment'),
#            ('OP2', 'env2', 'Make this an forest environment'),
#            ('OP3', 'ocean', 'Make this a ocean environment')
#        ]
#    )
...
#        bpy.ops.object.delete() # this shld delete all objects (+animation) in beginning
#        bpy.ops.outliner.orphans_purge(do_local_ids=True, do_linked_ids=True, do_recursive=False)
#        clear_objs = ["Plane", "Camera"]
#        for obj in bpy.data.objects:
#                if obj.name in clear_objs:
#               rx     bpy.data.objects.remove(obj, do_unlink=True)
            
#        self.remove_camera_markers() # remove any existing Torus camera markers
        bpy.ops.object.select_all(action='DESELECT')
...
#        default_objs = ["Camera", "Sun","Plane"]
#        for obj in collection:
#            if obj.name not in default_objs:
#                 obj.select_set(True)
...
#                bpy.data.particles[-1].size_random = 0.5 # param (0 to 0.5)
        ...
        self.remove_camera_markers() # remove Torus corner markers
